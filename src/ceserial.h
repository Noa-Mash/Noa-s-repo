...
public:
	...
	int ReadBuffer(char* data, int maxLen);
    int Available();
    void FlushBuffer();
};
...
inline int ceSerial::Available() {
    ...
    // Read available characters into temp buffer
    while (bufferCount < BUFFER_SIZE && bytesRead < 256) {
        char c = ReadChar(success);
        if (success) {
            tempBuf[bytesRead++] = c;
        } else {
            break;
    ...
    // Add temp buffer data to circular buffer
    for (int i = 0; i < bytesRead && bufferCount < BUFFER_SIZE; i++) {
        readBuffer[bufferEnd] = tempBuf[i];
...
inline int ceSerial::ReadBuffer(char* data, int maxLen) {
    if (!IsOpened() || !data || maxLen <= 0) return 0;
    
    // Update buffer with any new data
    Available();
    
    int bytesToRead = (maxLen < bufferCount) ? maxLen : bufferCount;
    
    for (int i = 0; i < bytesToRead; i++) {
        data[i] = readBuffer[bufferStart];
...
inline void ceSerial::FlushBuffer() {
    bufferStart = 0;
    bufferEnd = 0;
    bufferCount = 0;
}
